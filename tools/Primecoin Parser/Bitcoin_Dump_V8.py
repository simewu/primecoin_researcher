...
bitcoinPath =  '/media/ubuntu1/Blockchains/Primecoin/blocks'

startHeight = 0
endHeight = 4790923


bitcoinPath = os.path.expanduser(bitcoinPath)
if not os.path.exists(bitcoinPath):
	print(bitcoinPath + ' does not exist.')
	sys.exit()

blockchain = Blockchain(bitcoinPath)
output = open(os.path.expanduser(os.path.join('~', 'Desktop', 'Bitcoin full data V8.csv')), 'w+')
counter = 0
...
blockchain_index_path = os.path.join(bitcoinPath, 'index')
