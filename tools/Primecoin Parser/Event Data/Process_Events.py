...
def getMonthNum(month):
	...
	if month.endswith(' (early)') or month.endswith(' (late)'):
		month = month.split(' ')[0]
	if month.startswith('early ') or month.startswith('late '):
		month = month.split(' ')[1]
...
if __name__ == '__main__':
		...
		for row in reader:
			...
			# January (early)
				...
				if month_day.endswith(' (late)') or month_day.startswith('late '):
					start_date = datetime.date(year, month, 20)
				if month_day.endswith(' (early)') or month_day.startswith('early '):
			...
			# January - February
				...
				if month1_str.endswith(' (late)') or month1_str.startswith('late '):
					start_date = datetime.date(year, month1, 20)
				if month1_str.endswith(' (early)') or month1_str.startswith('early '):
					start_date = datetime.date(year, month1, 10)

				end_date = datetime.date(year, month2, num_days_in_month2)
				
				if month2_str.endswith(' (late)') or month2_str.startswith('late '):
					end_date = datetime.date(year, month2, 20)
				if month2_str.endswith(' (early)') or month2_str.startswith('early '):
